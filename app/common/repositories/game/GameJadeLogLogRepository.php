...
use app\common\dao\game\GameJadeLogDao;
...
class GameJadeLogLogRepository extends BaseRepository
{
    const LOG_TYPE = [
        1 => '后台调整',
        2 => '充值',
    ...
    public function __construct(GameJadeLogDao $dao)
    {
        $this->dao = $dao;
    }
    ...
    public function addJade($num,$userInfo,$companyId){
        /** @var UsersRepository $usersRepository */
//        $amount = explode('|',$num)[3];
//        $amount = base64_decode($amount);
//        if(!$amount || !is_numeric($amount)) throw new ValidateException('数量错误!');
        ...
        $todayRole = $gameJadeLogLogRepository->search(['uuid'=>$userInfo['id']],$companyId)->whereTime('add_time','today')->count('id');
         if($todayRole >0) throw new  ValidateException('今日已完成!');
        /** @var UsersPoolRepository $usersPoolRepository */
        $usersPoolRepository = app()->make(UsersPoolRepository::class);
        $amount = 60;
       $count =  $usersPoolRepository->search(['uuid'=>$userInfo['id'],'status'=>1],$companyId)->count('id');
       if($count > 0) $amount = 120;

       return $usersRepository->jadeChange($userInfo['id'],4,$amount,['remark'=>'战场收获'],4,$companyId);
    }
}